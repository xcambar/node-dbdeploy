...
changelog.prototype.build = function () {
    ...
    var me = this;
    _client.query(_createScript, function (err) {
        if (err) {
 ...
 * The parameter 'migrations' is expected to be an object literal with filenames as keys.
 * Values will be an object literal with two keys: 'do' and 'undo'
...
changelog.prototype.apply = function (migrations, direction) {
    ...
    var _applyAtomicMigration = function (statement, filename, number) {
        _client.query('BEGIN;');
        _client.query(statement, function (err) {
            if (err) {
                _client.end();
                throw 'migration from ' + filename + ' failed for the following reason: ' + err.message;
            }
        });
